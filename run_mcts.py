...
for epoch in range(3):
    for idx,x in enumerate(queries[0:200]):
        print('----',idx,'-----')
        realTime ,pg_plan_time, pg_latency, mcts_time, hinter_plan_time, MPHE_time, hinter_latency, actual_plans, actual_time = hinter.hinterRun(x[0])
        pg_latency/=1000
        hinter_latency/=1000
        pg_plan_time/=1000
