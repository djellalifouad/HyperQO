...
from testJoin import traverse_tree
...
class Hinter:
    ...
    def findBestHint(self, plan_json_PG, alias, sql_vec, sql):
        ...
        plan_times = self.predictWithUncertaintyBatch(plan_jsons=plan_jsons, sql_vec=sql_vec,index=1,leading_list=leading_list)
        ...
        return chosen_leading_pair,leading_list.index(chosen_leading_pair[1])
    def hinterRun(self, sql):
        file = open('scaler.txt', 'w')
        file.close()
        plan_json_pg = None
        plan_json_hybrid = None
        self.hinter_times += 1
        ...
        if config.cost_test_for_debug:
            ...
            plan_json_pg = pgrunner.getAnalysePlanJson(sql)
        ...
        plan_times = self.predictWithUncertaintyBatch(plan_jsons=plan_jsons, sql_vec=sql_vec,index=0)
        algorithm_idx = 0
        chosen_leading_pair,index_leading = self.findBestHint(plan_json_PG=plan_json_PG, alias=alias, sql_vec=sql_vec, sql=sql)
        ...
        plan_json_hybrid = pgrunner.getCostPlanJson(sql+chosen_leading_pair[1])
        plan_json_pg = pgrunner.getCostPlanJson(sql)
        ...
        return self.pg_planningtime_list[-1], self.pg_runningtime_list[-1], self.mcts_time_list[-1], \
               ...
               self.chosen_plan[-1], self.hinter_time_list[-1],plan_json_pg,plan_json_hybrid,chosen_leading_pair[1],index_leading

    def predictWithUncertaintyBatch(self, plan_jsons, sql_vec,index=0,leading_list=[]):
