...
class JoinTree:
    def __init__(self, sqlt, db_info, max_column_in_table=15, max_alias=40, extent_sql=True):
        ...
        for comparison in self.comparison_list:
            ...
            if len(comparison.aliasname_list) == 2:
                ...
                left_table_class = db_info.name2table[left_fullname]
                table_idx = left_table_class.column2idx[comparison.column_list[0]]
                self.table_fea_set[left_aliasname][table_idx * 2 + 1] = max(
                    self.table_fea_set[left_aliasname][table_idx * 2 + 1],
                    pgrunner.getSelectivity(str(self.aliasname2fromtable[comparison.aliasname_list[0]]),
                                            str(comparison)))
                self.alias_selectivity[self.aliasname2id[left_aliasname]] = max(
                    self.alias_selectivity[self.aliasname2id[left_aliasname]],
                    pgrunner.getSelectivity(str(self.aliasname2fromtable[comparison.aliasname_list[0]]),
                                            str(comparison)))

        for aliasname in self.aliasnames_root_set:
            self.table_fea_set[aliasname] = torch.tensor(self.table_fea_set[aliasname], device=device).reshape(1,
                                                                                                               -1).detach()
        ...
        self.join_matrix = torch.tensor(np.concatenate(
            (np.asarray(self.join_matrix).reshape(1, -1), np.asarray(self.alias_selectivity).reshape(1, -1)), axis=1),
    ...
    def toSql(self, ):
        root = self.total - 1
        if config.use_hint:
            print(self.hint(root))
            self.hintlist = "/*+\nLeading(" + self.hint(root) + ")\n*/\n"
            with open("./rtos_learned_query_optimizer/HintLatest.txt", "w") as text_file:
                leading = self.hint(root).replace("("," ").replace(")","").lstrip().rstrip()
                text_file.write(leading)
