...
from rtos_learned_query_optimizer.sqlSample import JoinTree
...
def Floss(value, targetvalue):
    ...
    return torch.mean(disl2 * (
                (1 - disl1) * torch.abs(value - targetvalue) * (targetvalue + 1) + disl1 * (value - targetvalue) * (
                    value - targetvalue)))
...
class ENV(object):
    ...
    def reward_new(self, filename='optionnal'):
        from math import e
        if self.sel.total + 1 == len(self.sel.from_table_list):
            if self.run_mode == False:
                ...
                #elf.create_file("rtos-solutions/", filename, sql)
...
class DQN:
    ...
    def validate_ind(self, val_list, tryTimes=1):
        ...
        for sql in val_list:
            ...
            env = ENV(sql, self.db_info, pgrunner, self.device)
            if (
                    len(env.sel.from_table_list) < 3):  # or not env.sel.baseline.left_deep or (sql.bestLatency is not None and sql.bestLatency>config.baselineValue)) and not config.testGen:
                rewards.append(-1.0)
                continue
